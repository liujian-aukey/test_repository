...
- **需求文档录入方式**：通过 **上传文件** 添加（支持批量上传、文件数量单次最多上传20个，单个文件大小限制为100M）。
- **校验（必填）**：  
  - 项目名称
  - 版本号
  - 项目经理
...
- 行为：跳转至 **二级菜单「需求文档管理」** 页，并 **定位到** 与该项目/版本对应的 **需求文档目录**（与 6.2 目录树「项目名称 → 版本号 」结构一致，确保打开后选中的树节点、右侧表单与目标文档所在目录一致）。
...
- 主文案：`当前目录下无需求文档`  
- 提供按钮：**立即上传**  
- 行为：选择本地文件上传到**当前目录**，支持批量上传文件（文件数量单次最多上传20个，单个文件大小限制为100M），上传成功后**刷新**右侧列表（进入 6.2.3 有文档态）。
...
1. 需求文档在树第三层是单文档还是多文档/文件夹。  
2.  可预览格式边界：例如 rp、旧版 Office、ZIP 的列表或禁止预览。  
3. 后续多用户、角色、审计、存储配额为二期，本期不写死。
...
从**项目概况 → 文档查看** 跳转时，需带**项目、版本、目标目录** 参数以正确定位。
