...
- **需求文档录入方式**：通过 **上传文件** 或 **选择本地文件夹上传** 添加；若上传文件夹，则按**文件夹结构**在对应**版本目录**下**还原/合并**为子目录与文件（与 **§6.2.2** 可延展树一致；数量与单文件/总大小等限制由评审与方案确定）。
- **校验（必填）**：  
  - 项目名称  
...
- **跳转目标**（**明确**）：从项目概况进入时，**仅**跳转到**需求文档管理**页中、与该行项目对应的**二级目录**——即树节点 **「项目名称」→「该行的版本号」**（**版本号**这一层，为该项目下该版本的**根目录/挂载点**）。**不**强制定位到更深层子目录；用户可在该版本下**自行展开**子文件夹继续浏览（见 **§6.2.2**）。
- **行为补充**：打开 **「需求文档管理」** 标签页后，**自动选中/展开** 至 **「项目 → 该版本号」** 节点，**右侧**展示该**版本目录**下内容（**直接**子级：文件或子文件夹在 **§6.2.3** 的列表/树中体现）。
...
- 若**当前节点**为**空目录**（**无**直挂文件、**且** 树上也**无** 子节点），主文案：`当前目录下无需求文档`，**立即上传** 可上传**文件**或**文件夹**（**文件夹上传** 行为同 **§6.2.2**）。
- 若**有子文件夹**但**本层无直挂文件**：可展示「当前层无直挂文件」类提示，**仍**提供**立即上传** 与 **在左侧** **展开/进入** 子夹的引导；**不** 强制要求有文件才**显示** 树，**以左侧树** 为准在**子目录** 下展示文件表。
- **上传** 到**当前目录** 时：支持**选择** **多个文件**、**单个文件夹** 或**混合**；完成后**刷新** 树与右侧列表（进入 6.2.3 或保持树可展开态）。
...
3. 上传文件夹/多文件的**单批上限**、**单文件/总大小** 上限、**重名**时**覆盖** 还是**自动重命名**。  
4. 后续多用户、角色、审计、存储配额为二期，本期不写死。

> 已在本版 PRD 中**纳入**：「项目名称+版本号」**唯一**；**文件夹上传** 与**可延展**目录树；**跳转** 至**版本号** 二级目录。
...
| 需求部门 | 是 | 文本 | 否* | 查询（模糊） |
| 描述 | 是 | 长文本 | 否 | 可摘要，弹窗全显 |
| 需求文档 | 是 | 关联/附件 | 视业务 | 标签块展示；与**版本**下**目录/文件** 的映射，见 A.1.4 |
...
**项目概况 → 文档查看**：**定位** 至 **L1=项目、L2=该版本号** 节点（**不** 强制 deeper）；参数至少含 **项目标识 + 版本号/版本目录 ID**。
